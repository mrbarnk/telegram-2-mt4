...
app = Client("mt5_signals", api_id="1047128",
             api_hash="1402e406ccfaf6d7ca2a08469bc60558")
...
def sltp(chat_id, text, Sl, Tp):
    ...
    try:
        try:
            ...
            try:
                PRICE1 = float(correct_number(re.findall(
                    r'[\d.]+', str(text.split('\n')[0]))[0]))
                PRICE1 = PRICE[0]
            except:
                Entry = "@"
                PRICE = re.findall(
                    r'[\d.]+', str([i for i in text.lower().split('\n') if Entry in i]))
                PRICE1 = PRICE[0]
                # print('price 1 error')
            # except:
            ...
            SL = float(correct_number(re.findall(
                r'[\d.]+', str([i for i in text.split('\n') if Sl in i]))[-1]))
            # TP1 = float(correct_number(re.findall(r'[\d.]+', str([i for i in text.split('\n') if Tp in i][-1]))[-1]))

            splited_texts = re.findall(
                r'[\d.]+', str([i for i in text.lower().split('\n') if Tp in i]))
            sls = re.findall(
                r'[\d.]+', str([i for i in text.lower().split('\n') if "sl" in i]))
            splited_texts = list(filter(lambda splited_text: splited_text not in [
                                 '1', '2', '3', '4', '5'], splited_texts))
            # print(splited_texts, 'PRICE1, PRICE2', sls)

            OPEN_TP = re.findall('open', str(
                [i for i in text.lower().split('\n') if "tp" in i]))
                        ...
                        if pair["symbol"].lower() in text:
                            ...
                            for entry in st['entries']:
                                if st is not False and Type != 150:
                                    for tp in st['tps']:
                                        print('Entry no: ', entry, 'TP: ', tp, 'TPLEN', len(
                                            st['tps']), 'Symbol: ', Symbol)
                                        try:
